...
const BASE_API_URL = 'https://mgmtapi.nagarkaryavalinew.com/';
...
const decryptResponse = (payload) => {
  try {
    ...
    console.warn("⚠️ Skipping decryption. Response might be plain JSON:", payload);
...
const apiService = {
  ...
  get: async (endpoint, params = {}) => {
    ...
    return fetch(`${BASE_API_URL}/proxy/${btoa(endpoint)}?payload=${encodeURIComponent(encryptedData)}`, {
      method: 'GET'
    })
      .then(res => res.json())
      .then(response => {
  ...
  getRawFile: async (encodedPath) => {
    try {
      const response = await fetch(`${BASE_API_URL}/proxy/public-file/${encodedPath}`);
