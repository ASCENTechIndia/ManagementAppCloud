...
const PieChart = ({
 ...
 const [showTable, setShowTable] = useState(false);
  ...
  const chartDataToRender = data?.length > 0
  ? {
      labels: data.map(item => item.name),
      datasets: [
        {
          data: data.map(item => item.total), 
          backgroundColor: themeColors.additionalColors.slice(0, data.length),
          borderColor: "#fff",
          borderWidth: 1,
        },
      ],
  ...
  <div className="mt-5">
    <table className="table table-bordered">
      <thead>
        <tr>
          <th>Category</th>
          <th>{chartMode === "yearly" ? "Years" : chartMode === "monthly" ? "Months" : "Days"}</th>
        </tr>
      </thead>
      <tbody>
        {chartDataToRender?.labels.map((label, idx) => (
          <tr key={idx}>
            <td>{label}</td>
            <td>{chartDataToRender.datasets[0].data[idx]}</td>
          </tr>
        ))}
      </tbody>
    </table>
