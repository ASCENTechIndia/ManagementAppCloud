...
const BirthRegistrationByWard = () => {
  ...
  const tableHeaders = ["प्रभागाचे नाव", "अर्ज", "अधिकृत"];
  const tableKeyMapping = {
    "प्रभागाचे नाव": "zonename",
    अर्ज: "appcount",
    अधिकृत: "appauthcount",
  };
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      setLoading(true);
      const payload = {
        Request1: `${import.meta.env.VITE_FLAG}$birthdeath_ZoneSummary$${userId}$${ulbid}~${formatDateForAPI(values.from)}~${formatDateForAPI(values.to)}`,
        Request2: "a",
        Request3: "a",
        Request4: "a",
        Request5: "a",
      ...
      if (
        ...
        const rows = res.data.data.jsondata.map((item) => ({
          ...
          appauthcount: Number(item.appauthcount) || 0,
        }));

        const totalApp = rows.reduce((sum, row) => sum + row.appcount, 0);
        const totalAuth = rows.reduce((sum, row) => sum + row.appauthcount, 0);

        const totalRow = {
          zonename: "एकूण",
          appcount: totalApp,
          appauthcount: totalAuth,
        };
...
export default BirthRegistrationByWard;
