...
const DeathRegistrationByDate = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      setLoading(true);
      const payload = {
        Request1: `${flag}$death_Summary$${userId}$${ulbid}~${formatDateForAPI(values.from)}~${formatDateForAPI(values.to)}`,
        Request2: "a",
        Request3: "a",
        Request4: "a",
        Request5: "a",
      ...
      if (
        ...
        const rows = res?.data?.data?.jsondata.map((item) => ({
          date: formatDate(item.applictndt),
          app: item.deathappcount,
          enroll: item.deathappauthcount
        }));

        const totalAppCount = rows.reduce((sum, row) => sum + row.app, 0);
        const totalEnrollCount = rows.reduce((sum, row) => sum + row.enroll, 0);
        const totalRow = {
          date: "एकूण",
          app: totalAppCount,
          enroll: totalEnrollCount
        };
...
export default DeathRegistrationByDate;
